...
import { Settings, Building2, Clock, CreditCard, Bell, Globe, RefreshCw, Save, Upload, Trash2, Calendar, Link, User } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-6">
      ...
      <div className="card p-6">
        ...
        {activeTab === 'general' && (
          <div className="space-y-6">
            ...
            {/* ── NOUVEAU : Nom du médecin ── */}
            <div className="p-4 bg-primary-50 border border-primary-200 rounded-xl">
              <div className="flex items-center gap-2 mb-3">
                <User size={18} className="text-primary-600" />
                <h3 className="font-semibold text-primary-800">Médecin responsable</h3>
              </div>
              <p className="text-xs text-primary-600 mb-3">
                Ce nom apparaîtra sur les factures PDF et dans le formulaire de rendez-vous.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    Nom complet du médecin <span className="text-primary-500">*</span>
                  </label>
                  <input
                    type="text"
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input type="tel" value={config.phone || ''} onChange={(e) => set('phone', e.target.value)} className="input-field" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={config.website || ''} onChange={(e) => set('website', e.target.value)} className="input-field" />
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'schedule' && (
          <div className="space-y-4">
            <div className="mb-6">
              ...
              <select
                value={config.appointmentDuration || 30}
                onChange={(e) => set('appointmentDuration', Number(e.target.value))}
                className="select-field w-48"
              >
            ...
            <div className="space-y-3">
              {days.map(day => {
                ...
                const updateDay = (patch) => setConfig(prev => ({
                  ...prev,
                  workingHours: { ...prev.workingHours, [day.id]: { ...dayConfig, ...patch } }
                }));
                return (
                  <div key={day.id} className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                    <label className="flex items-center gap-2 w-32">
                      <input type="checkbox" checked={dayConfig.enabled} onChange={(e) => updateDay({ enabled: e.target.checked })} className="checkbox-custom" />
                      <span className="font-medium text-slate-700">{day.label}</span>
                    </label>
                    <div className="flex items-center gap-2">
                      <input type="time" value={dayConfig.start} onChange={(e) => updateDay({ start: e.target.value })} disabled={!dayConfig.enabled} className="input-field w-32" />
                      <span className="text-slate-500">à</span>
                      <input type="time" value={dayConfig.end} onChange={(e) => updateDay({ end: e.target.value })} disabled={!dayConfig.enabled} className="input-field w-32" />
                    </div>
                  </div>
                );
              })}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={config.reminderSettings?.enabled ?? true}
                  onChange={(e) => setConfig(prev => ({ ...prev, reminderSettings: { ...prev.reminderSettings, enabled: e.target.checked } }))}
              ...
              <div className="grid grid-cols-3 gap-3">
                {REMINDER_TYPES.map(type => (
                  <button key={type.id} type="button"
                    onClick={() => setConfig(prev => ({ ...prev, reminderSettings: { ...prev.reminderSettings, defaultType: type.id } }))}
                    className={`p-4 rounded-xl border-2 transition-all ${config.reminderSettings?.defaultType === type.id ? 'border-primary-500 bg-primary-50' : 'border-slate-200 hover:border-slate-300'}`}>
            ...
            <div>
              ...
              <select value={config.reminderSettings?.defaultTiming || '24h'}
                onChange={(e) => setConfig(prev => ({ ...prev, reminderSettings: { ...prev.reminderSettings, defaultTiming: e.target.value } }))}
                className="select-field">
            ...
            <div>
              ...
              <textarea value={config.reminderSettings?.smsTemplate || ''}
                onChange={(e) => setConfig(prev => ({ ...prev, reminderSettings: { ...prev.reminderSettings, smsTemplate: e.target.value } }))}
                className="input-field" rows={2} />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Template WhatsApp</label>
              <textarea value={config.reminderSettings?.whatsappTemplate || ''}
                onChange={(e) => setConfig(prev => ({ ...prev, reminderSettings: { ...prev.reminderSettings, whatsappTemplate: e.target.value } }))}
        ...
        {activeTab === 'billing' && (
          <div className="space-y-6">
            <div>
              ...
              <input type="text" value={config.invoiceSettings?.prefix || 'FAC'}
                onChange={(e) => setConfig(prev => ({ ...prev, invoiceSettings: { ...prev.invoiceSettings, prefix: e.target.value } }))}
            ...
            <div>
              ...
              <input type="number" value={config.taxRate || 0}
                onChange={(e) => set('taxRate', Number(e.target.value))}
                className="input-field w-32" min="0" max="100" />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Coordonnées bancaires</label>
              <textarea value={config.invoiceSettings?.bankDetails || ''}
                onChange={(e) => setConfig(prev => ({ ...prev, invoiceSettings: { ...prev.invoiceSettings, bankDetails: e.target.value } }))}
                className="input-field" rows={2} placeholder="IBAN, BIC..." />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Pied de page factures</label>
              <textarea value={config.invoiceSettings?.footer || ''}
                onChange={(e) => setConfig(prev => ({ ...prev, invoiceSettings: { ...prev.invoiceSettings, footer: e.target.value } }))}
                className="input-field" rows={2} placeholder="Merci de votre confiance..." />
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="p-4 bg-gradient-to-r from-violet-50 to-purple-50 rounded-xl border border-violet-200">
              <div className="flex items-center gap-3 mb-3">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={config.stripeEnabled || false}
                    onChange={(e) => set('stripeEnabled', e.target.checked)} className="sr-only peer" />
              ...
              {config.stripeEnabled && (
                <div className="mt-3">
                  ...
                  <input type="text" value={config.stripePublicKey || ''} onChange={(e) => set('stripePublicKey', e.target.value)} className="input-field" placeholder="pk_live_..." />
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-gradient-to-r from-sky-50 to-blue-50 rounded-xl border border-sky-200">
              <div className="flex items-center gap-3 mb-3">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={config.googleCalendarEnabled || false}
                    onChange={(e) => set('googleCalendarEnabled', e.target.checked)} className="sr-only peer" />
              ...
              {config.googleCalendarEnabled && (
                <div className="mt-3">
                  ...
                  <input type="text" value={config.googleCalendarId || ''} onChange={(e) => set('googleCalendarId', e.target.value)} className="input-field" placeholder="votre-email@gmail.com" />
                </div>
              )}
            </div>
      ...
      <ConfirmModal
        ...
        message="Cette action supprimera toutes vos données et restaurera les données de démonstration. Cette action est irréversible."
